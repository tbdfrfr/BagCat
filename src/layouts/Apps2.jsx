...
const AppCard = memo(({ app, onClick, fallbackMap, onImgError, itemStyles, isPopular = false }) => {
  ...
  return (
    <div
      ...
      className={clsx(
        'flex-shrink-0',
        itemStyles.app,
        'bg-[rgba(20,29,43,0.56)] backdrop-blur-md border border-white/20',
        isPopular && 'm-0',
...
const CategoryRow = memo(({ category, games, onClick, onViewMore, fallback, onImgError, styles, isPopular = false }) => {
  ...
  return (
    <div className="mb-3 max-w-7xl mx-auto px-9">
      ...
      <div
        ...
        className={clsx(
          isPopular
            ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center pb-2 max-w-[84rem] mx-auto'
            : 'flex gap-1 overflow-x-auto pb-2 -ml-3 scrollbar-hide',
        )}
...
const Games = memo(() => {
  ...
  const popularGames = useMemo(() => data.Popular || [], [data]);
  ...
  const filtered = useMemo(() => {
    ...
    if (query) {
      toFilter = toFilter.filter((game) => game.appName.toLowerCase().includes(query));
    }
  ...
  }, [all, apps, data, category, showDl, dlGames, q, page, perPage]);
  ...
  return (
    <div className={`${styles.appContainer} w-full mx-auto`}>
      <div className="w-full px-4 py-4 flex justify-center mt-3 relative min-h-7">
        {(category || showDl) && (
          <button
            onClick={handleBack}
